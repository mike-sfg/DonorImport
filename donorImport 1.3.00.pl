...
# Constants
use constant DONORIMPORT_VERSION => "1.3.00";
...
FILE: while(my $input_filename = shift(@ARGV)) {
    ...
    if ((!$monthly_OK) and ($filetype eq 'monthly')) {
        $dialog = "DonorImport has detected that this file is a monthly giving ".
            ...
            "files for the same month's date range can result in duplicate " .
            "donations in your TntConnect database.\n";
        $title = "Alert";
    ...
    foreach my $line (<$input_fh>)
    {
        ...
        #           '00' => '(00) Work support'
        #
        # If also appends any comments from the donor if present in the remarks field
        my $gift_memo = '';
        ...
        # This list must be in the same order as the TNTMPDIG* constants
